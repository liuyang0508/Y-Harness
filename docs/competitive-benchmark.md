...
`adapter_conformance` result, one real Pi fixed-output result, and two Codex
CF-003 fault records. All have `claim_eligible: false`: they prove bounded
adapters and registered fault behavior, not that either Harness is better. See
...
3. ~~add OpenCode and Hermes Agent adapters without importing their code;~~
   ...
   real process-restart CF-003 record; Grok Build, OpenCode, and Hermes remain
   contract-only;
...
locally fault-tested in Y-Harness. Four real released-product records, the
