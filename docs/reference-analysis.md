...
| Model and Provider | `packages/ai` registers a broad built-in catalog. `models.json` can add OpenAI Chat/Responses, Anthropic Messages, Google Generative, and compatible local or remote endpoints without changing Pi source. | The service accepts reusable Provider Profiles plus a strict Model catalog and explicit 1–16 identity route. First-party adapters implement OpenAI Responses, OpenAI Chat Completions-compatible, Anthropic Messages, and Gemini `generateContent`; each independently maps native streaming, Tool calls, usage, and required opaque continuation. Chat profiles select current/legacy token-limit fields, optional stream usage, public HTTPS, or an explicit literal-loopback-IP HTTP exception for local inference. One Profile binds its Secret and transport policy for any number of Model aliases. Gateway and settlement-v1 JSON-command Models remain explicit escape hatches. Protocol v37 and the TUI expose only a credential-free active catalog. | **Aligned on the four principal wire families and configuration-driven compatible endpoints, not on every vendor auth/catalog feature.** Operators can add models and rotate API-key mappings without Rust changes while retaining stronger route and origin-bound replay evidence. Azure deployment mapping, Bedrock/Vertex signing, OAuth, automatic local discovery, command-Broker provisional streaming, load/price policy, and Pi's larger curated ecosystem remain open; no broad-Provider parity claim is made. |
...
| Packages, Skills, and extensions | `pi install/remove/list/update/config` manages npm, git, URL, local, user, project, and temporary sources. Packages may provide executable TypeScript extensions, Skills, prompts, and themes; `/reload` hot-loads resources. | `src/skill` verifies exact identity, dependencies, budgets, digests, publisher signatures, revocation, and transparency evidence. `yh skill` and `yh package` add raw-digest-pinned public Catalog search and named public/private Registries with per-request Bearer resolution, exclusive private CA support, exact package-origin allowlists, signed recursive acquisition, yanked/cycle/size checks, immutable Catalog cache and source receipts to install/list/verify/activate/deactivate/remove/history/rollback. Explicit upgrade preflights and atomically activates; TUI `/reload` switches only at a settled-Turn process-generation boundary and reattaches durable State. | **Aligned for declarative Skill discovery, exact dependency acquisition, authenticated Registry transport, governed update/rollback, and reload authority—not for source/ecosystem breadth.** Y-Harness deliberately excludes ambient package code and mixed-generation Turns. npm/git/local-temporary source ergonomics, Registry mirror federation, OAuth, executable-extension isolation, and ecosystem size still lag Pi. |
| Session and product surface | JSONL session trees support resume, tree navigation, fork, clone, compaction, import/export, and RPC. `getTree()` defensively assembles the entry DAG; `createBranchedSession` extracts the root-to-leaf path, preserves entry identities, re-chains retained entries, records `parentSession`, and writes the new file incrementally. `navigateTree()` may summarize the abandoned old-leaf suffix and append that derived text on the selected branch. | Authoritative SQLite Thread/Turn/Item State, recovery, checkpoints, bounded lineage-aware recent-Thread Protocol paging, explicit durable names, Protocol-backed TUI resume, schema-10 portable archives, schema-11 attributed per-Turn Context, schema-12 durable Thread/Operation tenant fencing, and schema-13 immutable execution binding are implemented. Fork preserves historical evidence identity and exact tenant ownership; format-3 archive import is atomic, rebinds unbound history, and refuses to rewrite tenant-bound execution evidence. Format-1 Thread handoff preparation computes a bounded source-only Turn delta, binds it to source/target identities and a canonical digest, and converts any host-generated candidate into attributed content-free Context evidence. | **Partially aligned by a different State model.** Durable fork/clone-at-head, bounded recent-page forests, integrity-bound import/export, tenant isolation, and read-only Thread-handoff preparation are Engine-owned. Pi's entry-level mutable leaf and automatic in-session navigation are deliberately rejected as a second branch authority. Candidate synthesis remains provider/host-selected and its factual quality requires evaluation. JSON is a bounded interchange format, not authoritative State. |
...
| Skill Engine | Codex and Claude Code have marketplace/plugin product flows; OpenCode pulls remote skills; Pi packages skills/extensions; Hermes ships a large catalog; Grok Build exposes Skills, Plugins, Hooks, and marketplace code in one product runtime. | Signature, revocation, transparency receipts, exact HTTPS pins, bounded public Catalog and named request-authenticated private Registry discovery, signed dependency acquisition, immutable cache/source receipts, and governed install/activate/update/rollback/remove are implemented. Mirror federation, OAuth, npm/git/temporary-source ergonomics, executable-extension isolation, and ecosystem size still lag. |
...
### Completion is stronger than Loop termination

A source-pinned audit on 2026-08-03 found useful completion-related mechanisms
in every reference, but no general atomic receipt binding the final candidate,
its exact Model request, the active capability generation, current verifier
outcomes, and durable evidence. The audited coordinates were Codex
`25af12f7e61572b0bc18ddb1008be543b91519b0`, Pi
`cee5ff7520d8828bed9955ef00419e995d1f91e0`, OpenCode
`7534d23551f665e65080809975b4ca5c7d63807b`, Hermes Agent
`3ef6bbd201263d354fd83ec55b3c306ded2eb72a`, SWE-agent
`abd7d69724d1413b30fea43d4724bb5b463906b4`, and the supplied Claude Code
reconstruction `3da94d5e5f2b99c9d82b0d8f09448b04775cd41f`.

The adopted parts remain deliberately separate:

- Pi contributes a clear Turn lifecycle, ordered concurrent Tool settlement,
  and safe handling of truncated Tool calls; its `settled` event is not task
  verification.
- Codex contributes typed Turn/Item lifecycle, rollout reconstruction, flush
  boundaries, and client reconciliation; `TurnComplete` does not prove tests
  or requested outcomes passed.
- OpenCode contributes pre/post workspace snapshots, Patch provenance,
  structured-output validation, and explicit interrupted Tool settlement;
  `idle` remains a process state.
- Hermes contributes verification freshness after workspace edits, bounded
  verify-on-stop, staged final answers, run fencing, and artifact existence
  checks; its evidence ledger explicitly is not by itself a completion gate.
- SWE-agent contributes the separation of trajectory, submission Artifact,
  reviewer selection, and salvaged failure output; `submitted` does not prove
  the task is solved.
- The Claude reconstruction contributes continuation-capable Stop Hooks and
  synthesis of missing Tool results. Its own README says it is incomplete and
  reconstructed from a distributed package, so it is behavioral evidence, not
  authoritative source and not code to copy.

Y-Harness therefore distinguishes Model/Loop termination, durable Turn
settlement, task completion decision, verifier evidence, and the final
CompletionReceipt. ADR 0157 makes a Model `EndTurn` only a candidate and places
channel delivery and Memory/title/experience derivation after the atomic
completion boundary. This is a design target until its migration, recovery,
fault-injection, and protocol tests pass; it is not a superiority claim.
...
### Completion-semantics audit additions

- [Pi loop settlement at `cee5ff7`](https://github.com/earendil-works/pi/blob/cee5ff7520d8828bed9955ef00419e995d1f91e0/packages/agent/src/agent-loop.ts)
- [Pi JSONL session persistence at `cee5ff7`](https://github.com/earendil-works/pi/blob/cee5ff7520d8828bed9955ef00419e995d1f91e0/packages/agent/src/harness/session/jsonl-storage.ts)
- [Codex Turn lifecycle at `25af12f`](https://github.com/openai/codex/blob/25af12f7e61572b0bc18ddb1008be543b91519b0/codex-rs/core/src/session/turn.rs)
- [Codex rollout reconstruction at `25af12f`](https://github.com/openai/codex/blob/25af12f7e61572b0bc18ddb1008be543b91519b0/codex-rs/core/src/session/rollout_reconstruction.rs)
- [OpenCode workspace snapshots at `7534d23`](https://github.com/anomalyco/opencode/blob/7534d23551f665e65080809975b4ca5c7d63807b/packages/opencode/src/snapshot/index.ts)
- [OpenCode completion processor at `7534d23`](https://github.com/anomalyco/opencode/blob/7534d23551f665e65080809975b4ca5c7d63807b/packages/opencode/src/session/processor.ts)
- [Hermes verification freshness ledger at `3ef6bbd`](https://github.com/NousResearch/hermes-agent/blob/3ef6bbd201263d354fd83ec55b3c306ded2eb72a/agent/verification_evidence.py)
- [Hermes bounded verify-on-stop at `3ef6bbd`](https://github.com/NousResearch/hermes-agent/blob/3ef6bbd201263d354fd83ec55b3c306ded2eb72a/agent/verification_stop.py)
- [SWE-agent execution and submission lifecycle at `abd7d69`](https://github.com/SWE-agent/SWE-agent/blob/abd7d69724d1413b30fea43d4724bb5b463906b4/sweagent/agent/agents.py)
- [SWE-agent submission artifact tool at `abd7d69`](https://github.com/SWE-agent/SWE-agent/blob/abd7d69724d1413b30fea43d4724bb5b463906b4/tools/submit/bin/submit)
- [Claude reconstruction Stop Hooks at `3da94d5`](https://github.com/liuyang0508/claude-code-source-code/blob/3da94d5e5f2b99c9d82b0d8f09448b04775cd41f/src/query/stopHooks.ts)
