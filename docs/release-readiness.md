...
| Deterministic tests | 391 Core library, 8 Domain Pack control-plane, 11 CLI, 24 Engine process/service, 11 TUI unit/render, 2 private-gateway TLS integration, 2 private-MCP TLS integration, 53 product/Engine evidence-adapter, and 7 fault-fixture tests | passing locally: 509 total plus 8 explicitly ignored manual/environment fixtures |
...
| Distribution package | `cargo package --locked -p y-harness`, 239-file / 3.3 MiB clean-room Core crate verification (729.0 KiB compressed) | passing locally with State-13, Approval-3, Task-3, Secret-2, and Protocol-25 coordinates; optional control-plane crate remains a separate workspace package |
...
| Optional Domain Pack control plane | format/store schema 1, immutable exact component pins, mandatory pinned Evaluation suite, terminal evaluation, independent approval, tenant-partitioned release/activation records, SQLite CAS, bounded rollback, and execution-time inventory binding | canonical/tamper/inventory tests plus Memory/SQLite lifecycle, reopen, cross-tenant, projection-drift, conversion to generic Engine execution evidence, and two-connection CAS tests passing; remote lifecycle integration, role authorization, canary, and multi-node HA remain open |
