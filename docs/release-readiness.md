...
| Deterministic tests | 460 Core library, 13 Domain Pack control-plane, 15 CLI unit, 5 Protocol/CLI integration, 24 Engine process/service, 18 TUI unit/render, 2 private-gateway TLS integration, 2 private-MCP TLS integration, 53 product/Engine evidence-adapter, and 7 fault-fixture tests | passing locally: 599 total plus 8 explicitly ignored manual/environment fixtures; zero-default workspace passes 569 plus 4 ignored |
...
| Optional reference Effect consumer | strict config-schema-1 opt-in; independent execution/reconciliation tasks; separate exact Connector registries and non-empty allowlists; explicit trust origin/idempotency/read-only contracts; fixed service Authority; bounded cadence/backoff/concurrency/timeouts; disposable cursors; content-free health transitions; Effect-before-Temporal-before-Protocol/MCP shutdown | strict/default/duplicate/unsupported/timeout tests plus real `yh serve` commands reaching Unknown, emitting invalid-reconciliation degradation, recovering to Applied after target repair, stopping cleanly, and proving no terminal replay after restart; disabled by omission, no Core task or Protocol v29 command, executable content pin, target truth certification, durable circuit breaker, or multi-node leader claim |
