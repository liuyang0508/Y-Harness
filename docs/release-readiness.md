...
| Tenant-owned Thread State | schema-12 authoritative creation evidence plus disposable validated projections; exact Authority tenant on Thread, Turn, recovery, fork, handoff, archive, and retained Operation access | Memory/SQLite cross-tenant denial, archive rebind, protocol capability fail-closed, and SQLite reopen tests passing; Approval, Task, Secret, Artifact, and Domain Pack ownership remain open |
