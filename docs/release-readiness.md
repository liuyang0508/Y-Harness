...
| Deterministic tests | 385 Core library, 8 Domain Pack control-plane, 11 CLI, 24 Engine process/service, 11 TUI unit/render, 2 private-gateway TLS integration, 2 private-MCP TLS integration, 53 product/Engine evidence-adapter, and 7 fault-fixture tests | passing locally: 503 total plus 8 explicitly ignored manual/environment fixtures |
...
| Distribution package | `cargo package --locked -p y-harness`, 238-file / 3.3 MiB clean-room Core crate verification (721.2 KiB compressed) | passing locally with State-13, Approval-3, Task-2, Secret-2, and Protocol-24 coordinates; optional control-plane crate remains a separate workspace package |
...
| Durable Turn execution binding | schema-13 single content-free Item with trusted actor, exact configuration/environment SHA-256, revision, and tenant; Model-invisible, snapshot/reopen durable, archive-safe, exact on approval continuation | constructor/unknown-field/schema gate, pre-State tenant denial, duplicate denial, SQLite snapshot/reopen, Model invisibility, approval missing/substitution, Domain Pack conversion, and archive rebind-denial tests passing; Task-attempt binding remains open |
| Optional Domain Pack control plane | format/store schema 1, immutable exact component pins, mandatory pinned Evaluation suite, terminal evaluation, independent approval, tenant-partitioned release/activation records, SQLite CAS, bounded rollback, and execution-time inventory binding | canonical/tamper/inventory tests plus Memory/SQLite lifecycle, reopen, cross-tenant, projection-drift, conversion to Engine Turn evidence, and two-connection CAS tests passing; remote lifecycle integration, role authorization, Task-attempt binding, canary, and multi-node HA remain open |
