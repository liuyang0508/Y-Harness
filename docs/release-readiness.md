...
| Released-product evidence tools | external-run formats 1/2/3/4/5/6 plus Codex CF-003 format 7; exact adapter/product/source/fixture hashes; bounded Claude Code JSON, Codex JSONL, Grok Build headless JSON, Pi JSONL, OpenCode JSONL, and Hermes one-shot/usage processes; deterministic loopback Responses Provider; unavailable facts preserved as unavailable; exact-pinned MCP crash-after-effect fixture with durable oracle | 39 adapter/fixture tests plus real Claude Code 2.1.143 fixed-output and Codex 0.145.0 CF-003 records passing; both explicitly ineligible for comparison claims, and Grok Build, Pi, OpenCode, and Hermes remain contract-only |
