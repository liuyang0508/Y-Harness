...
| Deterministic tests | 436 Core library, 13 Domain Pack control-plane, 15 CLI, 28 Engine process/service, 17 TUI unit/render, 2 private-gateway TLS integration, 2 private-MCP TLS integration, 53 product/Engine evidence-adapter, and 7 fault-fixture tests | passing locally: 573 total plus 8 explicitly ignored manual/environment fixtures; zero-default workspace passes 543 plus 4 ignored |
| Full-screen TUI PTY | demo and configured Engine modes; real Turn, atomic Thread fork, durable State, alternate screen and bracketed-paste restoration; exact protocol mismatch reports both coordinates and same-checkout reinstall commands; empty/short conversation hierarchy, nonzero sub-percent capacity, and durable `local/demo` labeling are render-tested | debug and release binaries passing |
