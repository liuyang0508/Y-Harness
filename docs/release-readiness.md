...
| Deterministic tests | 253 library, 1 CLI, 7 Engine process/service, 10 TUI unit/render, and 2 private-gateway TLS integration tests | passing |
...
| Distribution package | `cargo package --locked -p y-harness`, clean-room crate verification | 164 files, 1.9 MiB unpacked / 433.3 KiB compressed; passing |
...
- An authenticated, exact-versioned HTTPS JSON model-gateway adapter and
  ...
  bounded provisional deltas and requires a final typed response. Direct vendor
  adapters and a live external gateway pass are not yet implemented; ignored
  environment-gated ordinary and streaming TLS round-trip tests define the
