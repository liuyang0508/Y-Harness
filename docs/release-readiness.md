...
| Deterministic tests | 344 library, 10 CLI, 23 Engine process/service, 11 TUI unit/render, 2 private-gateway TLS integration, 2 private-MCP TLS integration, 41 product/Engine evidence-adapter, and 6 fault-fixture tests | passing locally: 439 total plus 7 explicitly ignored fixtures |
...
| Product and Engine evidence tools | external-run formats 1–6, Codex CF-003 formats 7/8, and Y-Harness CF-003 format 9; exact adapter/product/source/fixture/rollout/config hashes; bounded Claude Code JSON, Codex JSONL, Grok Build headless JSON, Pi JSONL, OpenCode JSONL, Hermes one-shot/usage, and typed Y-Harness stdio processes; deterministic loopback Responses Provider or spec-bound JSON-command Model; unavailable facts preserved as unavailable; exact-pinned MCP crash/hold-after-effect fixture with durable oracle | adapter/fixture tests plus real Claude Code 2.1.143 fixed-output, Codex 0.145.0 single-process/same-Thread restart, and Y-Harness explicit-recovery restart CF-003 records passing; all explicitly ineligible for comparison claims, with an identity-bound detached-MCP release marker, new-Turn recovery, no implicit takeover, and no descendant-exit claim recorded, while Grok Build, Pi, OpenCode, and Hermes remain contract-only |
