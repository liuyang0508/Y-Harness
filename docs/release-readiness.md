...
| Deterministic tests | 341 library, 10 CLI, 23 Engine process/service, 11 TUI unit/render, 2 private-gateway TLS integration, 2 private-MCP TLS integration, 29 released-product adapter, and 4 fault-fixture tests | passing locally: 422 total plus 7 explicitly ignored fixtures |
...
| Distribution package | `cargo package --locked -p y-harness`, 221-file clean-room crate verification | passing locally from the committed clean tree |
...
| Released-product evidence tools | external-run formats 1/2/3/4/5/6; exact adapter/product hashes; bounded Claude Code JSON, Codex JSONL, Grok Build headless JSON, Pi JSONL, OpenCode JSONL, and Hermes one-shot/usage processes; unavailable facts preserved as unavailable; requested/observed Model and estimated/actual cost separation; exact-pinned MCP crash-after-effect fixture with durable oracle | 33 adapter/fixture tests and one real Claude Code 2.1.143 conformance record passing; Codex, Grok Build, Pi, OpenCode, and Hermes have contract evidence only; all explicitly ineligible for comparison claims |
