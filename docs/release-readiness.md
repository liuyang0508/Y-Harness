...
| Deterministic tests | 446 Core library, 13 Domain Pack control-plane, 15 CLI unit, 5 Protocol/CLI integration, 24 Engine process/service, 18 TUI unit/render, 2 private-gateway TLS integration, 2 private-MCP TLS integration, 53 product/Engine evidence-adapter, and 7 fault-fixture tests | passing locally: 585 total plus 8 explicitly ignored manual/environment fixtures; zero-default workspace passes 555 plus 4 ignored |
...
| Distribution package | `cargo package --locked -p y-harness`, clean-room Core crate verification | passing locally with State-14, Approval-3, Task-3, Workflow-1, Handoff-1, Effect-1, embedded Temporal-2, Secret-2, and Protocol-29 coordinates; optional control-plane crate remains a separate workspace package |
...
| Durable Effect Ledger | independent schema-1 aggregate; immutable bounded request and digest, tenant/capability/operation/idempotency uniqueness, positive-attempt worker leases, applied/rejected/unknown states, fail-closed expiration, exact reconciliation, content-free receipts, actor-and-tenant-bound command digests, revision CAS, immutable transitions, Memory/SQLite parity, conditional Protocol v29 surface | aggregate projection/digest/owner/expiry/idempotency tests, tenant fencing, revision/tenant projection drift, SQLite reopen/partial-store validation, two-connection claim CAS, protocol capability/conflict/content-light list tests, service restart recovery, and Temporal exact-boundary expiry pass; automatic execution, compensation, receipt truth verification, encryption/retention, distributed scheduling, and channel delivery remain open |
...
- Workflow schema 1 provides durable, fenced Run state and evidence above a
  ...
  now records and reconciles unknown external outcomes independently, but no
  host automatically executes pending Effects, verifies arbitrary external
  receipts, retries Tasks, executes compensation plans, or routes Human
  Handoff. SQLite provides single-host/multiprocess CAS, not distributed
