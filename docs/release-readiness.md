...
| Read-only service-store preflight | `doctor` and `serve` validate existing State/Approval/Task/Workflow/Handoff stores before external capability construction; concrete adapters use read-only `query_only`; missing stores remain bootstrap-eligible; migration remains explicit and backup-first | fresh/current-store status, legacy State no-mutation/provider-order, and partial Workflow fail-closed process tests passing; authoritative service open still revalidates |
| Feature isolation | zero-default core, each optional feature, and all features | passing |
| Deterministic tests | 436 Core library, 13 Domain Pack control-plane, 15 CLI, 28 Engine process/service, 12 TUI unit/render, 2 private-gateway TLS integration, 2 private-MCP TLS integration, 53 product/Engine evidence-adapter, and 7 fault-fixture tests | passing locally: 568 total plus 8 explicitly ignored manual/environment fixtures; zero-default workspace passes 538 plus 4 ignored |
| Full-screen TUI PTY | demo and configured Engine modes; real Turn, atomic Thread fork, durable State, alternate screen and bracketed-paste restoration; exact protocol mismatch reports both coordinates and same-checkout reinstall commands | debug and release binaries passing |
| Installed operator path | isolated-prefix Engine and TUI installs; version, init, doctor, persistent service, demo, Task DAG, Mailbox, durable Workflow Run, and Human Handoff | passing; TUI install contains only `yh-tui`; Task Graph terminal at revision 6 and Workflow/Handoff restart recovery are process-tested |
| Distribution package | `cargo package --locked -p y-harness`, clean-room Core crate verification | passing locally with State-14, Approval-3, Task-3, Workflow-1, Handoff-1, embedded Temporal-1, Secret-2, and Protocol-28 coordinates; optional control-plane crate remains a separate workspace package |
...
- Workflow schema 1 provides durable, fenced Run state and evidence above a
  ...
  embedded Temporal Driver can advance a bounded page of due waits when a host
  invokes it, but the reference service does not yet run that loop,
  automatically retry Tasks, reconcile unknown effects, execute durable
  compensation plans, or automatically route Human Handoff. SQLite provides
  single-host/multiprocess CAS, not
...
- Human Handoff schema 1 provides durable, lease-fenced ownership state over
  ...
  person. An embedding host may use Temporal Driver API 1 for bounded expiry.
