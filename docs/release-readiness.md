...
| Deterministic tests | 341 library, 10 CLI, 23 Engine process/service, 11 TUI unit/render, 2 private-gateway TLS integration, 2 private-MCP TLS integration, 35 released-product adapter, and 4 fault-fixture tests | passing locally: 428 total plus 7 explicitly ignored fixtures |
...
| Distribution package | `cargo package --locked -p y-harness`, 222-file clean-room crate verification | passing locally from the committed clean tree |
...
| Released-product evidence tools | external-run formats 1/2/3/4/5/6 plus Codex CF-003 formats 7/8; exact adapter/product/source/fixture/rollout hashes; bounded Claude Code JSON, Codex JSONL, Grok Build headless JSON, Pi JSONL, OpenCode JSONL, and Hermes one-shot/usage processes; deterministic loopback Responses Provider; unavailable facts preserved as unavailable; exact-pinned MCP crash/hold-after-effect fixture with durable oracle | 43 adapter/fixture tests plus real Claude Code 2.1.143 fixed-output and Codex 0.145.0 single-process/same-Thread restart CF-003 records passing; all explicitly ineligible for comparison claims, with detached MCP cleanup and new-Turn recovery recorded, while Grok Build, Pi, OpenCode, and Hermes remain contract-only |
