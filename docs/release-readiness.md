...
| Released-product evidence tools | external-run formats 1/2/3; exact adapter/product hashes; bounded Claude Code JSON, Codex JSONL, and Grok Build headless JSON processes; unavailable facts preserved as unavailable; requested/observed model and cost separation; exact-pinned MCP crash-after-effect fixture with durable oracle | 19 tests and one real Claude Code 2.1.143 conformance record passing; Codex and Grok Build have contract evidence only; all explicitly ineligible for comparison claims |
