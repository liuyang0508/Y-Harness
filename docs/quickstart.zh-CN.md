...
`yh-tui` 只通过 Protocol v24 调用 `yh`：它不读取 SQLite，不构造
...
## 4. 运行 Protocol v24 服务

`serve` 通过 stdin/stdout 读写一行一个 JSON 的 Protocol v24 帧：
...
```bash
printf '%s\n' \
  '{"id":"init-1","protocol_version":"24","command":{"method":"initialize"}}' \
  | yh serve y-harness.json
```
