...
`yh-tui` 只通过 Protocol v27 调用 `yh`：它不读取 SQLite，不构造
...
## 4. 运行 Protocol v27 服务

`serve` 通过 stdin/stdout 读写一行一个 JSON 的 Protocol v27 帧：
...
四个 SQLite 权威库均使用 WAL、`synchronous=FULL` 和精确 schema
校验。服务重启后 Thread、Task Graph 与 Workflow Run 均可恢复。
