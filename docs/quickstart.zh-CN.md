...
`yh-tui` 只通过 Protocol v23 调用 `yh`：它不读取 SQLite，不构造
...
## 4. 运行 Protocol v23 服务

`serve` 通过 stdin/stdout 读写一行一个 JSON 的 Protocol v23 帧：
...
```bash
printf '%s\n' \
  '{"id":"init-1","protocol_version":"23","command":{"method":"initialize"}}' \
  | yh serve y-harness.json
```
