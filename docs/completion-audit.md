...
| Tool Runtime | typed registry, MCP, process/CLI adapters, default-deny bounded MCP launch, Unix process-group settlement, reusable macOS sandbox, explicit compensation | `src/execution`, `src/transport/mcp.rs`, ADRs 0013/0036/0043/0054/0056/0062/0066 | local baseline passing; escape-resistant Linux plus Linux/Windows persistent containment open |
| State Engine | typed journal, memory/SQLite stores, CAS, checkpoints, snapshots, capacity/recovery bounds, schema-1/2/3 to schema-4 backup-first migration | `src/state`, schema-1/2/3/4 fixtures, crash-phase and maximum-size migration tests, ADRs 0061/0065/0068 | local baseline passing; archival/offload open |
| Memory Engine | versioned provider port, scoped provenance, Agent Memory Hub MCP adapter | `src/memory`, unit tests and environment-gated real MCP test | adapter and sandboxed local round trip passing; remote CI environment-gated |
...
| Evaluation | bounded parallel cases/graders, isolated failures, format-2 root validation, origin-bound exact baselines, required-pass gates, versioned end-to-end smoke suite | `src/evaluation`, `evals`, `yh eval-smoke`, ADRs 0010/0026/0064/0067/0069 | executable local baseline passing |
...
The following commands passed on 2026-07-25 with Rust 1.88:
...
Agent Memory Hub, ordinary and streaming HTTPS model gateway, and pinned HTTPS
Skill acquisition. The Agent Memory Hub test passed against the local
