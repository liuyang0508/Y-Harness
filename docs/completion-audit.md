...
| Orchestration | bounded DAG, executable TaskExecutor scheduler, dependency concurrency, timeout/panic isolation, leases/fencing, paged TaskMailbox messaging, default-deny Workspace Provider lifecycle, isolated local directories, pinned detached Git Worktrees, Artifacts, memory and SQLite coordination, durable optional Task Graph tenant ownership, append-only exact-attempt execution binding with retry anti-downgrade, serviceable authenticated worker protocol, an independent schema-1 Workflow Run aggregate with revision CAS and fenced waits, an independent schema-1 Human Handoff aggregate with stable queue order, actor-bound idempotency, finite owner leases and unique claim fences, plus embedded Temporal Driver API 1 for bounded host-driven due advancement | `src/orchestration`, `src/workflow`, `src/human_handoff`, `src/temporal`, `src/protocol/task.rs`, `src/protocol/workflow.rs`, `src/protocol/human_handoff.rs`, migration/restart/tamper/conflict/tenant-fencing/temporal-race tests, `examples/orchestrated.rs`, ADRs 0011/0019/0052/0053/0071/0072/0073/0074/0119/0123/0127/0128/0129 | embedded governed and protocol tenant-fenced Task/Workflow/Handoff lifecycles plus bounded host ticks pass for single-host/multiprocess coordination; a resident polling lifecycle, automatic effect-safe retry, durable compensation planning, automatic channel routing/outbox, remote binding control/evidence inspection, multi-node consensus, tenant transfer, and durable orphan reconciliation remain open |
...
3 manual size tests, 13 Domain Pack control-plane tests, 12 CLI configuration
tests, 25 Engine process/service tests, 11 TUI unit/render tests, 2 local
...
fault-fixture tests: 561 passing plus 8 explicitly ignored fixtures in total.
The no-default-feature workspace run contains 531 passing tests plus 4 ignored
