...
| Evaluation | bounded parallel cases/graders, exact per-Grader cancellation, configured brokered JSON-command Graders, isolated in-memory `yh eval`, format-2 root validation, origin-bound exact baselines, required-pass gates, versioned end-to-end smoke suite, external-run formats 1/2/3/4/5/6 plus Codex CF-003 formats 7/8, controller-owned deterministic Tool fault oracle | `src/evaluation`, `src/execution`, `evals`, `yh eval`, `yh eval-smoke`, `tools/benchmark-runner`, ADRs 0010/0026/0064/0067/0069/0079/0080/0081/0082/0090/0107/0109/0110/0111/0112 | executable built-in and configured-process baselines, bounded Claude Code, Codex, Grok Build, Pi, OpenCode, and Hermes Agent adapter contracts, one real non-claim Claude Code record, and real non-claim Codex single-process plus same-Thread restart uncertain-effect records |
...
| Competitive benchmark tools | independent `y-harness-benchmark-runner` and `y-harness-fault-fixture` packages; bounded shell-free Claude Code JSON, Codex JSONL, Grok Build headless JSON, Pi JSONL, OpenCode JSONL, and Hermes one-shot/usage adapters; exact binary coordinates; external-run formats 1/2/3/4/5/6 plus Codex CF-003 formats 7/8; deterministic stdio MCP crash/hold-after-effect fixture and durable oracle | 43 adapter/fixture tests plus real `claim_eligible: false` Claude fixed-output and Codex single-process/restart CF-003 records; no comparative case exists |
...
private-MCP TLS integration tests, 35 released-product adapter tests, and 4
deterministic fault-fixture tests: 428 passing plus 7 explicitly ignored
fixtures in total. The no-default-feature workspace run contains 399 passing tests plus 3 ignored
