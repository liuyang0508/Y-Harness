...
| Evaluation | bounded parallel cases/graders, isolated failures, format-2 root validation, origin-bound exact baselines, required-pass gates, versioned end-to-end smoke suite, external-run format 1, controller-owned deterministic Tool fault oracle | `src/evaluation`, `evals`, `yh eval-smoke`, `tools/benchmark-runner`, ADRs 0010/0026/0064/0067/0069/0079/0080 | executable local baseline, one real non-claim Claude Code adapter-conformance record, and one non-claim crash-after-effect fixture |
...
| Competitive benchmark tools | independent `y-harness-benchmark-runner` and `y-harness-fault-fixture` packages; bounded shell-free Claude Code JSON adapter; exact binary coordinates; external-run format 1; deterministic stdio MCP crash-after-effect fixture and durable oracle | 8 adapter/fixture tests plus one real `claim_eligible: false` Claude conformance result; no comparative case result |
