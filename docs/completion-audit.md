...
| Orchestration | bounded DAG, executable TaskExecutor scheduler, dependency concurrency, timeout/panic isolation, leases/fencing, paged TaskMailbox messaging, default-deny Workspace Provider lifecycle, isolated local directories, pinned detached Git Worktrees, Artifacts, memory and SQLite coordination, durable optional Task Graph tenant ownership, append-only exact-attempt execution binding with retry anti-downgrade, serviceable authenticated worker protocol, an independent schema-1 Workflow Run aggregate with revision CAS and fenced waits, and an independent schema-1 Human Handoff aggregate with stable queue order, actor-bound idempotency, finite owner leases, and unique claim fences | `src/orchestration`, `src/workflow`, `src/human_handoff`, `src/protocol/task.rs`, `src/protocol/workflow.rs`, `src/protocol/human_handoff.rs`, migration/restart/tamper/conflict/tenant-fencing tests, `examples/orchestrated.rs`, ADRs 0011/0019/0052/0053/0071/0072/0073/0074/0119/0123/0127/0128 | embedded governed and protocol tenant-fenced Task/Workflow/Handoff lifecycles passing for single-host/multiprocess coordination; background timer/lease polling, automatic effect-safe retry, durable compensation planning, automatic channel routing/outbox, remote binding control/evidence inspection, multi-node consensus, tenant transfer, and durable orphan reconciliation remain open |
...
The all-feature workspace run contains 425 passing Core library tests plus
...
fault-fixture tests: 550 passing plus 8 explicitly ignored fixtures in total.
The no-default-feature workspace run contains 520 passing tests plus 4 ignored
...
State-14/Approval-3/Task-3/Workflow-1/Handoff-1/Secret-2/Protocol-28 candidate; Cargo's
