...
| Competitive benchmark tools | independent `y-harness-benchmark-runner` and `y-harness-fault-fixture` packages; bounded shell-free Claude Code JSON, Codex JSONL, Grok Build headless JSON, Pi JSONL, and OpenCode JSONL adapters; exact binary coordinates; external-run formats 1/2/3/4/5; deterministic stdio MCP crash-after-effect fixture and durable oracle | 27 adapter/fixture tests plus one real `claim_eligible: false` Claude conformance result; Codex, Grok Build, Pi, and OpenCode have no live records and no comparative case exists |
...
private-MCP TLS integration tests, 23 released-product adapter tests, and 4
deterministic fault-fixture tests: 416 passing plus 7 explicitly ignored
fixtures in total. The no-default-feature workspace run contains 387 passing tests plus 3 ignored
...
The 220-file package archive verifies from the committed clean-tree candidate
