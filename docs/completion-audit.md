...
|---|---|---|
| Headless embeddable Rust core | public contracts in `src/lib.rs`; zero-default build; external-view examples execute a Policy-controlled Model/Tool loop, a fenced Task DAG, and a default-deny durable Effect Connector; embedded Temporal Driver API 2 and Effect Executor API 1 advance authoritative state without owning a host loop | compiled and run locally; CI-gated |
