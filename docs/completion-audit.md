...
| Optional full-screen TUI | separate `y-harness-tui` package and `yh-tui` binary in `clients/tui`; Protocol-v28-only child transport with actionable Engine/TUI coordinate mismatch diagnostics, execution-binding audit rendering, bounded tenant/lineage-aware recent-Thread navigation/resume, `/fork [terminal-turn-id]`, exact-ID active-Turn Steering, invalidated provisional-output handling, content-free continuation and ordered batch rendering, TestBackend render tests, real PTY Turn and fork | independently installable; local unit/lint/PTY gates passing; Pi-style entry-level in-place navigation is intentionally outside the Engine model |
...
tests, 28 Engine process/service tests, 12 TUI unit/render tests, 2 local
...
fault-fixture tests: 568 passing plus 8 explicitly ignored fixtures in total.
The no-default-feature workspace run contains 538 passing tests plus 4 ignored
