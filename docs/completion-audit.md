...
| Orchestration | bounded DAG, executable TaskExecutor scheduler, dependency concurrency, timeout/panic isolation, leases/fencing, paged TaskMailbox messaging, default-deny Workspace Provider lifecycle, isolated local directories, pinned detached Git Worktrees, Artifacts, memory and SQLite coordination, durable optional Task Graph tenant ownership, and serviceable authenticated worker protocol | `src/orchestration`, `src/protocol/task.rs`, schema-1 migration/restart/projection-drift/tenant-fencing tests, `examples/orchestrated.rs`, ADRs 0011/0019/0052/0053/0071/0072/0073/0074/0119 | embedded and protocol tenant-fenced worker lifecycles passing for single-host/multiprocess coordination; multi-node consensus, tenant transfer, and durable orphan reconciliation open |
...
| Optional full-screen TUI | separate `y-harness-tui` package and `yh-tui` binary in `clients/tui`; Protocol-v22-only child transport, bounded tenant/lineage-aware recent-Thread navigation/resume, `/fork [terminal-turn-id]`, exact-ID active-Turn Steering, invalidated provisional-output handling, content-free continuation and ordered batch rendering, TestBackend render tests, real PTY Turn and fork | independently installable; local unit/lint/PTY gates passing; Pi-style entry-level in-place navigation is intentionally outside the Engine model |
