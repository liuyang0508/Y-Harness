...
| Policy Engine | deny/allow/ask, risk class, trusted per-Turn actor/tenant Authority Context, attributed durable approval, restart-safe inbox, CAS, exact-actor separation of duty, fingerprinted continuation, backup-first schema migration | `src/runtime/policy.rs`, `src/approval`, Authority propagation and restart/drift tests, ADRs 0007/0024/0049–0051/0063/0065/0116 | local baseline passing; tenant-scoped approval deliberately fails closed until durable tenant evidence exists; roles and signed receipts open |
...
| Serviceable typed protocol | language-neutral v19 specification, exact envelope and schema-11/API-7 compatibility coordinates, panic-isolated trusted Authority resolution, permissioned expected-Turn recovery takeover, bounded per-Turn Context, lineage-aware Thread summaries, atomic retry-identified Thread forks, durable Thread names/import provenance, async Turn operations, exact-ID Steering, conditional Task Graph discovery, authenticated fenced worker lifecycle, bounded paging, cancellation, shutdown | `docs/protocol.md`, `src/protocol`, process/TLS/fault tests; passing locally |
...
| Optional full-screen TUI | separate `y-harness-tui` package and `yh-tui` binary in `clients/tui`; Protocol-v19-only child transport, bounded lineage-aware recent-Thread navigation/resume, `/fork [terminal-turn-id]`, exact-ID active-Turn Steering, invalidated provisional-output handling, content-free continuation and ordered batch rendering, TestBackend render tests, real PTY Turn and fork | independently installable; local unit/lint/PTY gates passing; Pi-style entry-level in-place navigation is intentionally outside the Engine model |
...
The all-feature workspace run contains 360 passing library tests plus
...
deterministic fault-fixture tests: 469 passing plus 7 explicitly ignored
fixtures in total. The no-default-feature workspace run contains 440 passing tests plus 3 ignored
