...
| External model provider | exact-versioned HTTPS JSON/NDJSON gateway plus direct OpenAI Responses JSON/SSE, secret references, exclusive private-CA trust and mTLS gateway identity | local private-gateway TLS and OpenAI mapping/stream tests passing; live gateway and OpenAI API environment-gated; opaque reasoning Tool continuation fails closed |
...
The all-feature workspace run contains 258 passing library tests, 2 CLI
