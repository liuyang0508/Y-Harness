...
| Orchestration | bounded DAG, executable TaskExecutor scheduler, dependency concurrency, timeout/panic isolation, leases/fencing, paged TaskMailbox messaging, default-deny Workspace Provider lifecycle, isolated local directories, pinned detached Git Worktrees, Artifacts, memory and SQLite coordination, durable optional Task Graph tenant ownership, append-only exact-attempt execution binding with retry anti-downgrade, serviceable authenticated worker protocol, independent schema-1 Workflow and Human Handoff aggregates, independent schema-1 Effect Ledger with tenant-scoped idempotency uniqueness, finite attempt leases, fail-closed unknown outcomes, explicit reconciliation and receipts, embedded default-deny Governed Effect Executor API 1, embedded default-deny Governed Effect Reconciler API 1, embedded Temporal Driver API 2 for bounded host-driven due advancement, and an opt-in reference-service polling lifecycle | `src/orchestration`, `src/workflow`, `src/human_handoff`, `src/effect`, `src/temporal`, `src/reference_cli/temporal_service.rs`, `src/protocol/{task,workflow,human_handoff,effect}.rs`, restart/tamper/conflict/tenant-fencing/temporal-race/service-lifecycle tests, `examples/{orchestrated,effect_executor,effect_reconciler}.rs`, ADRs 0011/0019/0052/0053/0071–0074/0119/0123/0127–0130/0133–0135 | embedded governed and protocol tenant-fenced Task/Workflow/Handoff/Effect lifecycles, bounded host ticks, fixed-authority service polling, bounded policy-gated Connector execution, and bounded policy-gated authoritative read-only unknown convergence pass for single-host/multiprocess SQLite coordination; configured service consumption, compensation execution, channel routing, external receipt verification, reconciliation cadence/backoff, multi-node query ownership, tenant transfer, and multi-node consensus remain open |
...
|---|---|---|
| Headless embeddable Rust core | public contracts in `src/lib.rs`; zero-default build; external-view examples execute a Policy-controlled Model/Tool loop, a fenced Task DAG, a default-deny durable Effect Connector, and authoritative read-only Effect reconciliation; embedded Temporal Driver API 2, Effect Executor API 1, and Effect Reconciler API 1 advance authoritative state without owning a host loop | compiled and run locally; CI-gated |
...
State-14/Approval-3/Task-3/Workflow-1/Handoff-1/Effect-1/Effect-Executor-1/Temporal-2/Secret-2/Protocol-29 candidate; Cargo's
