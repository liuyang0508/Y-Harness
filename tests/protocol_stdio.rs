...
#[test]
fn tui_client_drives_the_runtime_only_through_protocol_commands() {
    let working_directory = isolated_working_directory("tui");
    fs::create_dir_all(&working_directory).expect("create isolated working directory");
    ...
    let output = child.wait_with_output().expect("wait for TUI client");
    let _ = fs::remove_dir_all(&working_directory);
    assert!(
        output.status.success(),
        "TUI failed: {}",
    ...
    let stdout = String::from_utf8(output.stdout).expect("UTF-8 stdout");
    assert!(stdout.contains("assistant> Y-Harness observed tool output"));
...
fn state_migration_cli_creates_a_no_clobber_backup() {
    ...
    assert!(String::from_utf8_lossy(&output.stdout).contains("State schema 1 -> 4"));
