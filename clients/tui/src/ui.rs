...
fn render_transcript_empty_state(frame: &mut Frame<'_>, area: Rect) {
    let lines = vec![
        ...
        Line::default(),
        Line::styled(
            "The TUI is connected to the headless Engine through Protocol v30.",
...
fn render_help(frame: &mut Frame<'_>, area: Rect) {
    ...
    let help = vec![
        ...
        Line::styled(
            "Approvals and Tasks are accessed exclusively through Protocol v30.",
            Style::default().fg(MUTED),
        ),
        Line::raw(""),
...
mod tests {
    ...
    fn full_screen_renders_engine_projection_and_boundary() -> Result<(), Box<dyn std::error::Error>>
    {
        ...
        assert!(screen.contains("Keep clients behind Protocol v30"));
    ...
    -> Result<(), Box<dyn std::error::Error>> {
        ...
        assert!(screen.contains("headless Engine through Protocol v30"));
